...
const lessors = [
  ...
  {
    id: '2',
    img: 'Images/zombie-img.png',
    name: 'Mr. Zombie',
    icon: 'Images/grave-icon.png',
    quantity: '24 graves',
  },

  {
    id: '3',
    img: 'Images/dracula-img.png',
    name: 'Dracula',
    icon: 'Images/bat-icon.png',
    quantity: 'not defined',
  },

  {
    id: '4',
    img: 'Images/mummy-img.png',
    name: 'Mummy',
    icon: 'Images/coffin-icon.png',
...
function Lerrors() {
  return (
    <>
      <section id="lessors" className="mt-16 lg:mt-28 text-center px-4 sm:px-6 md:px-8 py-4 lg:px-12 2xl:max-w-[100rem] mx-auto scroll-m-32">
        <div className="">
          ...
          <p className="text-fright-textColor max-w-[23.125rem] w-full mx-auto opacity-75">
            Any lover of fear will find something here to immerse themselves in the spooky atmosphere.
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 place-items-center gap-4 xl:gap-12 my-8">
        {lessors.map((lessors, id) => (
          <div key={id} className="bg-fright-darkViolet bg-opacity-60 p-4 md:bg-transparent rounded-md sm:rounded-lg">
            <img 
              src={lessors.img} 
              alt={lessors.name} 
        ...
        <div className="flex flex-col justify-center items-center gap-4 md:gap-8">
          <p className="text-fright-textColor max-w-[23.125rem] w-full mx-auto opacity-75">
            Want to be one of them? Join us now for free!
            Your one drop of 🩸 is enough for us.
          </p>

          <button className="flex items-center gap-2 p-3 lg:py-3 lg:px-6 rounded-xl lg:rounded-2xl border border-fright-lightGradientPink bg-gradient-to-r 
