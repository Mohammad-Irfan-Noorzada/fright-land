...
function Header() {
  ...
  return (
    <>
      <header className="flex items-start md:flex-row justify-between md:items-center gap-4 py-4 md:py-6 lg:py-2 px-4 sm:px-6 md:px-8 lg:px-12 2xl:max-w-[100rem] mx-auto
      fixed top-0 left-0 right-0 z-20 scroll-m-28 bg-fright-bgColor shadow-[0_0.2rem_0.7rem_rgba(0,0,0,0.5)]">

        <div className={`md:hidden fixed w-screen h-screen inset-0 bg-black transition-all duration-700 z-10
          ${isOpen ? 'opacity-70 pointer-events-none' : 'opacity-0 pointer-events-none'}`}>
        </div>

        <div className="z-20">
          <nav className={`md:relative fixed top-0 left-0 h-full w-3/5 md:left-0 transition-all duration-700 pt-24 pl-8 md:pl-0 md:pt-0 bg-gradient-to-t 
            ...
            {isOpen && (
              <div className="absolute top-6 left-5 sm:left-6 text-3xl md:hidden" onClick={() => setIsOpen(false)}>
                <img 
        ...
        <div className="flex items-center text-fright-textColor gap-4 lg:gap-8 xl:gap-12">
          ...
          <div className="flex md:items-center gap-3 lg:gap-5 text-fright-whiteshColor">
            ...
            <button className="hidden sm:flex items-center gap-2 p-2 lg:py-3 lg:px-6 rounded-xl lg:rounded-2xl border border-fright-lightGradientPink bg-gradient-to-r 
        ...
        {!isOpen && (
          <div className="absolute top-5 left-4 sm:left-5 md:hidden" onClick={() => setIsOpen(true)}>
            <img 
              src="Images/menu.png" 
              alt="" 
